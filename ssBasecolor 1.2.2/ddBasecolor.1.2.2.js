class ddBasecolor {
	...
	*/
	constructor(color, cmyk = false) {
		this.version = "1.2.2"; // major . minor . small
	...
	*/
	_index(p, r, g = -1, b) {
		...
		for(var i=0;i<=100;i+=1) {
			...
			var tl = t.luminance();
			if (Math.abs(tl - l) > gap || j == 0) {
				s.push(j);
				l = tl;
			}
			if (j == 1 && Math.abs(tl - l) <= gap && s[s.length - 1] != 1) s.push(1);
		}
		var sc = p * (s.length-1);
		var p1 = s[Math.floor(sc)];
		var p2 = s[Math.ceil(sc)];if (j == 1 && Math.abs(tl - l) <= 0.02) s.push(1);
	...
	*/
	...
	distinct() { // smart
		// future
		// make a list of individually distinguishable colors
	}
	order() { // smart
	/*
	...
	*/
	grey(gr = 1, l = 0) { // smart
		...
		this._to_hsl();
		if(this._cmyk) this._to_cmyk();
        if(l) return this._smart_luminance(new ddBasecolor('white'), l);
		return this._resetbasecolor();
	}
	...
	*/
	index(m=2, i=1, incl, shift) {
		var inc = incl === false ? false : true;
		shift = isNaN(shift) && !isNaN(incl) ? incl : shift;
		if(!shift) shift = 0;
		if(isNaN(m) || m < 2) m = 2;
		m = inc ? m : m + 1;
		if(isNaN(i) || i < 1) i = 1;
	...
	*/
	...
	/*
	// Make color opaque by percentage
	*/
	opaque(o = 1) { // general
		o = this._max(o, 1);
